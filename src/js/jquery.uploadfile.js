...
(function ($) {
	...
	$.fn.uploadFile = function (options) {
		...
		if (s.dragDrop) {
			...
			$(this).before(dragDrop);
			$(dragDrop).append(uploadLabel);
			$(dragDrop).append($(s.dragDropStr));
			setDragDropHandlers(this, s, dragDrop);

		} else {
			$(this).before(uploadLabel);
		}

		this.formGroup = formGroup;

		createCutomInputFile(this, formGroup, s, uploadLabel);
		...
		function serializeAndUploadFiles(s, obj, files) {
			for (var i = 0; i < files.length; i++) {
				if (isFileTypeAllowed(obj, s, files[i].name)) {
					...
					$.fn.uploadFile.ajaxFormSubmit(form, ts, pd, fileArray);
					obj.fileCounter++;
				}
			...
			fileInput.change(function () {
				...
				if (this.files) //support reading files
				{
					for (i = 0; i < this.files.length; i++) {
						if (!isFileTypeAllowed(obj, s, this.files[i].name)) {
				...
				if (s.multiple && feature.formdata) //use HTML5 support and split file submission
				{
					...
					$.fn.uploadFile.ajaxFormSubmit(form, s, pd, fileArray);
				}
		...
		function createProgressDiv(obj) {
			...
			this.abort = $("<div class='ajax-file-upload-red'>Abort</div>").appendTo(this.statusbar).hide();
	...
	$.fn.uploadFile.ajaxFormSubmit = function (form, s, pd, fileArray) {
			...
			$.fn.ajaxSubmit = function (options) {
				...
				function fileUploadXhr(a) {
					...
					s.beforeSend = function (xhr, o) {
						if (options.fileData) //Fix for XHR2 form data
						{
							o.data = options.fileData;
						} else {
							o.data = formdata;
						}
						if (beforeSend)
